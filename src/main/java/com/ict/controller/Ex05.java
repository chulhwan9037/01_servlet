...
public class Ex05 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 서블릿의 첫번째 역할은 클라이언트의 요청을 받는 것이다 .
		// 요청에 따른 한글 처리
		...
		// 클라이언트가 요청할 때 사용하는 메서드는 get 방식, post 방식 이다. 
		// get 방식 : 주소창에 정보가 보인다. (정보를 헤더에 담아 보낸다.) (url~~?name=value&name=value~~)
		// post 방식 : 주소창에 정보가 안보인다. (정보를 바디에 담아 보낸다.)
		
		// 파라미터를 받아서 처리하는 메서드들 (1,2번 주로 사용)
		// 1. request.getParameter("name")       : String      
		// 2. request.getParameterValues("name") : String[] 
		
		// 맵 형식의 파라미터일 때 사용
		// 3. request.getParameterMap("name")    : Map<String, String[]>
