...
public class Ex10 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//           즉, 파라미터도 사라진다. 
		//           새로운 request, response 가 만들어 진다. 
		//           주소창에 최종 주소가 보인다. 
		//           response.sendRedirect("주소");
		
		// response.sendRedirect("Ex10_2"); 확장자가 없으면 서블릿
		// response.sendRedirect("view/ex09.jsp"); jsp로 이동
		
		
		// 포워딩 : 다른 페이지로 이동하면서 기존의 request, response 정보를 가지고 간다.
		//        파라미터도 사용가능 하다. 
		//        주소창에는 최초 주소가 보인다. 
