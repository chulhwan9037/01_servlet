...
<body>
	<%-- 
		pageContext 객체는 JSP 기본 내장객체이다. 
		JSP 페이지에서 1:1 로 연결된 객체. (JSP 페이지 하나당 pageContext 객체 하나 생성)
		같은 페이지에서 서로 값을 공유할 수 있으며 다른 기본 내장객체를 구하거나, 페이지의 흐름을 제어한다.  
	
		1. 데이터 저장 : pageContext.setAttribute("이름", 객체);
		   데이터 호출 : pageContext.getAttribute("이름"); => EL ${이름}
		   생명주기 : 해당 페이지를 벗어나면 저장 정보도 사라진다. 
		   
		** 속성을 이용해서 데이터를 저장 및 호출하는 내장 객체들 
			우선순위 : pageContext > request > session > application => 데이터 저장, 호출 방법은 같다.
		    ...
		    session : 해당 브라우저가 종료되면 정보도 사라진다. 
		    		  예외적으로 시간을 지정할 수 있다. => 팝업에서 일주일간 안보기, 일정시간 지나면 로그아웃
		    		  보통 로그인 처리할 때 사용
